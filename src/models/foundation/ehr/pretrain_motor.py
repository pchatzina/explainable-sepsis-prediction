...
See src/models/foundation/ehr/README.md for architecture details and output structure.
...
# Early stopping patience (number of consecutive evaluations without improvement)
# With EVAL_EVERY_STEPS=500 this translates to 2500 optimizer steps of patience,
# reasonable for convergence without excessive overfitting
...
def main() -> None:
    ...
    out_dir = Config.MODEL_EHR_MOTOR_PRETRAINING_OUT_DIR
    out_dir.mkdir(parents=True, exist_ok=True)

    # ------------------------------------------------------------------
    # 1. Load pretraining artifacts produced by prepare_motor.py
    # ------------------------------------------------------------------
    # All downstream components depend on these artifacts; missing any one
    # prevents training from proceeding (fail-fast pattern).
    ...
    # 4. Configure Trainer
    # ------------------------------------------------------------------
    # TrainingArguments: Central configuration for HuggingFace Trainer
    # Orchestrates all aspects: optimization, evaluation, checkpointing, logging
    ...
    # Automatically save and copy the best model bundle
    final_dir = out_dir / "final"
    trainer.save_model(str(final_dir))
    logger.info("Best model saved to %s", final_dir)

    model_dir = Config.MODEL_EHR_MOTOR_WEIGHTS_DIR
    ...
    for fname in ("config.json", "model.safetensors"):
        ...
        if src.exists():
            ...
            logger.error("Expected %s not found in final/ — skipping", fname)
    ...
    if dict_src.exists():
        ...
        logger.error("dictionary.msgpack not found at %s — skipping", dict_src)
