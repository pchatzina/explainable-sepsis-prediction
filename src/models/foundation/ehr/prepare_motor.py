...
See src/models/foundation/ehr/README.md for the full artifact list and idempotency notes.
...
# cohort splits defined in Phase 1).
TRAIN_FRAC = 0.90
VAL_FRAC = 0.10
...
# Tokenizer vocabulary size — 16384 (16 × 1024) is the MOTOR default.
VOCAB_SIZE = 16 * 1024
...
def build_or_load_ontology(
    database: meds_reader.SubjectDatabase, ontology_path: pathlib.Path
) -> femr.ontology.Ontology:
    ...
    logger.info("Building ontology (Athena: %s)", Config.MODEL_EHR_MOTOR_VOCAB_DIR)
    ontology = femr.ontology.Ontology(
        Config.MODEL_EHR_MOTOR_VOCAB_DIR, Config.PROCESSED_EHR_MEDS_METADATA_FILE
    )
...
def create_or_load_split(
    database: meds_reader.SubjectDatabase, split_path: pathlib.Path
) -> femr.splits.SubjectSplit:
    """Create a deterministic hash split or reload from CSV."""
    if split_path.exists():
        logger.info("Loading existing split from %s", split_path)
        return femr.splits.SubjectSplit.load_from_csv(split_path)
...
) -> femr.models.tokenizer.HierarchicalTokenizer:
...
def fit_or_load_motor_task(
    train_database: meds_reader.SubjectDatabase,
    tokenizer: femr.models.tokenizer.HierarchicalTokenizer,
    task_path: pathlib.Path,
) -> femr.models.tasks.MOTORTask:
...
def convert_or_load_batches(
    processor: femr.models.processor.FEMRBatchProcessor,
    database: meds_reader.SubjectDatabase,
...
def main() -> None:
    ...
    output_dir = Config.MODEL_EHR_MOTOR_PRETRAINING_FILES_DIR
    output_dir.mkdir(parents=True, exist_ok=True)

    logger.info("Opening MEDS database: %s", Config.PROCESSED_EHR_MEDS_PRETRAINING_DIR)

    with meds_reader.SubjectDatabase(
        str(Config.PROCESSED_EHR_MEDS_PRETRAINING_DIR), num_threads=NUM_PROC
    ) as database:
        # 1. Ontology
