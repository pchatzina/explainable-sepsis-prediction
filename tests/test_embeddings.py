...
class _BaseEmbeddingTests:
    ...
    def test_required_keys(self, embedding_data):
        ...
        for key in ("embeddings", "labels", "subject_ids"):
    ...
    def test_subject_ids_unique(self, embedding_data):
        data, _ = embedding_data
    ...
    def test_no_sample_id_leakage(self):
        ...
        for split in SPLITS:
            ...
            if not path.exists():
                pytest.skip("Not all splits present yet")
            data = torch.load(path, map_location="cpu", weights_only=False)
            all_ids.extend(data["subject_ids"])
    ...
    def test_data_types_are_integers(self, embedding_data):
        """Ensures that both labels and subject_ids are saved strictly as integers."""
        data, _ = embedding_data
        ...
        # Check subject_ids
        assert all(isinstance(sid, int) for sid in data["subject_ids"]), (
            "All subject_ids must be of type int (not str) for Late-Fusion alignment"
        )
...
class TestEHREmbeddings(_BaseEmbeddingTests):
    ...
    EMBEDDINGS_DIR = Config.PROCESSED_EHR_EMBEDDINGS_DIR
...
class TestECGEmbeddings(_BaseEmbeddingTests):
    ...
    EMBEDDINGS_DIR = Config.PROCESSED_ECG_EMBEDDINGS_DIR
...
class TestCXRImgEmbeddings(_BaseEmbeddingTests):
    ...
    EMBEDDINGS_DIR = Config.PROCESSED_CXR_IMG_EMBEDDINGS_DIR
...
class TestCXRTxtEmbeddings(_BaseEmbeddingTests):
    ...
    EMBEDDINGS_DIR = Config.PROCESSED_CXR_TXT_EMBEDDINGS_DIR


# ==========================================
# BASE EXTRACTOR UNIT TESTS
# ==========================================
import numpy as np
...
class DummyExtractor(BaseEmbeddingExtractor):
    """A minimal subclass to test the BaseEmbeddingExtractor's save_split method."""

    def extract_and_save(self) -> None:
        pass


def test_base_extractor_type_enforcement(tmp_path):
    """Verifies that save_split strictly requires PyTorch Tensors and rejects others."""
    extractor = DummyExtractor(output_dir=tmp_path)

    dummy_labels = [0, 1]
    dummy_subject_ids = [101, 102]

    # 1. Should PASS with a PyTorch Tensor
    valid_tensor = torch.tensor([[0.1, 0.2], [0.3, 0.4]], dtype=torch.float32)
    extractor.save_split("train", valid_tensor, dummy_labels, dummy_subject_ids)
    assert (tmp_path / "train_embeddings_raw.pt").exists(), (
        "Failed to save valid Tensor"
    )

    # 2. Should FAIL with a Python List
    invalid_list = [[0.1, 0.2], [0.3, 0.4]]
    with pytest.raises(ValueError, match="Embeddings must be a PyTorch Tensor"):
        extractor.save_split("valid", invalid_list, dummy_labels, dummy_subject_ids)

    # 3. Should FAIL with a NumPy Array
    invalid_numpy = np.array([[0.1, 0.2], [0.3, 0.4]], dtype=np.float32)
    with pytest.raises(ValueError, match="Embeddings must be a PyTorch Tensor"):
        extractor.save_split("test", invalid_numpy, dummy_labels, dummy_subject_ids)
