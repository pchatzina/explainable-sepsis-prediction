"""
Phase 1: Data Acquisition Pipeline Tests

This test suite validates the entire Phase 1 data acquisition and cohort construction.
...
Expected deterministic counts (thesis targets):
  - Total patients: 15,513 (after all filtering and deduplication)
  - Sepsis+ cases: 8,436
  - CXR studies: 2,212 (one per final cohort patient with CXR)
  - ECG studies: 7,146 (one per final cohort patient with ECG)

These counts ensure reproducibility across runs.
...
def cohort_stats(db_engine):
    """
    Fetch basic cohort statistics (total count, sepsis+ count) from database.
    Used by multiple tests to validate cohort integrity and class balance.
    """
...
def test_db_connection(db_engine):
    """
    Sanity check: Can we connect to the database?
    Validates that the PostgreSQL connection is working and tables exist.
    """
    with db_engine.connect() as conn:
...
def test_final_cohort_exists_and_has_data(cohort_stats):
    """
    Validate exact deterministic cohort size.
    """
...
def test_class_balance(cohort_stats):
    """
    Validate exact count of sepsis+ cases (reproducibility).
    """
...
def test_one_admission_per_patient(db_engine):
    """
    Verify strict 1:1 mapping of patient (subject_id) to admission (hadm_id).
    """
    with db_engine.connect() as conn:
        result = conn.execute(
            text("""
...
def test_cxr_cohort_integrity(db_engine):
    """
    Validate CXR modality:
      (1) Exact count: 2,212 CXR studies (expected from cohort definition)
      (2) Referential integrity: All CXRs link to existing patients (no orphans)
      (3) Uniqueness: At most 1 CXR per patient (deduplication enforced)
    """
    with db_engine.connect() as conn:
...
def test_ecg_cohort_integrity(db_engine):
    """
    Validate ECG modality:
      (1) Exact count: 7,146 ECG studies (expected from cohort definition)
      (2) Referential integrity: All ECGs link to existing patients (no orphans)
      (3) No referential integrity check here; orphan ECGs caught by FK constraint
    """
    with db_engine.connect() as conn:
...
def cxr_paths(db_engine):
    """
    Fetch all CXR file paths from database (module-scoped fixture).
    """
    with db_engine.connect() as conn:
        rows = conn.execute(
...
def ecg_paths(db_engine):
    """
    Fetch all ECG file paths from database (module-scoped fixture).
    """
    with db_engine.connect() as conn:
        rows = conn.execute(
...
def test_cxr_dir_exists():
    """
    Verify CXR image directory is accessible (CONFIG.RAW_CXR_IMG_DIR).
    """
...
def test_ecg_dir_exists():
    """
    Verify ECG signal directory is accessible (CONFIG.RAW_ECG_DIR).
    """
...
def test_cxr_files_on_disk(cxr_paths):
    """
    Verify all CXR image files referenced in database exist and are non-empty.
    """
...
def test_ecg_files_on_disk(ecg_paths):
    """
    Verify all ECG signal files (.hea header + .dat data) exist and are non-empty.
    """
...
def test_cxr_files_not_corrupted(cxr_paths):
    """
    Validate CXR image files are readable and not corrupted.
    """
    corrupted = []
    for db_path in cxr_paths:
        full_path = Config.RAW_CXR_IMG_DIR / db_path
...
def test_ecg_files_not_corrupted(ecg_paths):
    """
    Validate ECG signal files are readable WFDB format and not corrupted.
    """
    corrupted = []
    for db_path in ecg_paths:
...
def test_cxr_reports_zip_not_corrupted():
    """
    Validate CXR radiology reports ZIP archive is not corrupted.
    """
...
def test_extract_cohort_splits_creates_file(db_engine, tmp_path):
    """
    Validate extract_cohort_splits() creates master_cohort.parquet with correct schema.
    """
